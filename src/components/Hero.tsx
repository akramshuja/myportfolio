...
const Hero = () => {
  ...
  const contactInfo: ContactItem[] = [
    ...
    ...(Array.isArray(contactData.contactInfo) && contactData.contactInfo.some(item => item.icon === "github")
      ? []
      : [{
    ...
    ...(Array.isArray(contactData.contactInfo) && contactData.contactInfo.some(item => item.icon === "linkedin")
      ? []
      : [{
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6 mb-8">
            {contactInfo.map((item, idx) => {
              ...
              if (!href) {
                ...
                else if (item.icon === "linkedin") href = resolveLinkedin(item.value);
              }

              // For LinkedIn, ensure open in new tab no matter what
              const isExternal =
                item.icon === "github" || item.icon === "linkedin";
              return (
                <a
                  key={item.icon + idx}
                  href={href}
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-8 py-3 rounded-lg font-semibold hover:from-blue-700 hover:to-purple-700 transition-all duration-200 flex items-center gap-2 justify-center">
              <Download size={20} />
              Download Resume
            </button>
            <a
